...
def getTauWPWeight(df, working_point_tau1, working_point_tau2):
    ...
    df = df.Define(f"weight_tau1_TauID_SF_{working_point_tau1}",
                            f'''tau1_gen_kind == 5?
                            ::correction::TauCorrProvider::getGlobal().getSF_WPStrings(tau1_pt,  tau1_eta, tau1_decayMode,tau1_gen_kind, "{working_point_tau1}") : 1.;
                            ''')
    df = df.Define(f"weight_tau2_TauID_SF_{working_point_tau2}",
                            f'''tau2_gen_kind == 5?
                            ::correction::TauCorrProvider::getGlobal().getSF_WPStrings(tau2_pt,  tau2_eta, tau2_decayMode,tau2_gen_kind,"{working_point_tau2}") : 1.;
                            ''')
    df = df.Define(f"weight_TauID_new",f"""weight_tau1_TauID_SF_{working_point_tau1}*weight_tau2_TauID_SF_{working_point_tau2}""")
...
if __name__ == "__main__":
    ...
    background_string = {
        "0":"tau1_gen_kind!=5 || tau2_gen_kind!=5",
    ...
    for sample_name in signal_samples + bckg_samples :
        ...
        dataframes_channel = {
            ...
            #'tautau_Tight_VTight': df_tautau_VTight.Filter(f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Tight.value}"),
            ...
            #'tautau_VTight_Tight': df_tautau_Tight.Filter(f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.VTight.value}"),
            ...
            #'tautau_Loose_VTight': df_tautau_VTight.Filter(f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"),
            #'tautau_Loose_Tight': df_tautau_Tight.Filter(f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"),
            #'tautau_Loose_Medium': df_tautau_Medium.Filter(f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"),
            #'tautau_Loose_Loose': df_tautau_Loose.Filter(f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"),
            ##'tautau_Loose_VLoose': df_tautau_VLoose.Filter(f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"),
            ##'tautau_Loose_VVLoose': df_tautau_VVLoose.Filter(f"tau1_idDeepTau{deepTauYear}{args.deepTauVersion}VSjet >= {Utilities.WorkingPointsTauVSjet.Loose.value}"),
        ...
        for channel in dataframes_channel.keys():
            ...
            if sample_name in signal_samples:
                ...
                dataframes_channel[channel] = dataframes_channel[channel].Filter(f"sel_id == {args.bckg_sel} && ( tau1_gen_kind == 1 || tau2_gen_kind == 1 || tau1_gen_kind == 3 || tau2_gen_kind == 3) ")
            ...
            #sum_weights, sum_weights_error = getSumWeights(dataframes_channel[channel], trg_tauTau_list_application_fullOR,f'{total_weight_string}*weight_ditau_application*weight_singleTau_application*weight_MET_application')
    ...
    with open(f'Studies/Triggers/files/signals_{sampleName}{suffix}_VSeLoose.json', 'w') as f:
        json.dump(s_values, f, indent=4)
    if bckg_samples:
        with open(f'Studies/Triggers/files/backgrounds_{bckg_names}_{args.bckg_sel}{suffix}_VSeLoose_onlyElectrons.json', 'w') as f:
